...
class Controller:
    ...
    def changesystembrightness():
        ...
        if isinstance(brightness_levels, list):
            currentBrightnessLv = sum(brightness_levels) / len(brightness_levels) / 100.0
        else:
            currentBrightnessLv = brightness_levels / 100.0

        currentBrightnessLv += Controller.pinchlv / 50.0
        currentBrightnessLv = max(0.0, min(1.0, currentBrightnessLv))
        sbcontrol.fade_brightness(int(100 * currentBrightnessLv), start=int(100 * (brightness_levels if isinstance(brightness_levels, float) else sum(brightness_levels) / len(brightness_levels))))
